...
class HTML implements InterfaceTags
{
    ...
    public function __construct(string $lang = 'pt-BR', string $charset = 'UTF-8')
    {
        ...
        $this->addInHead(new Tag('meta', null, ['charset'=>[$this->charset]]));
        $this->addInHead(new Tag('meta', null, ['http-equiv'=>['X-UA-Compatible'], 'content'=>['IE=edge']]));
        $this->addInHead(new Tag('meta', null, ['name'=>['viewport'], 'content'=>['width=device-width, initial-scale=1']]));
    }
